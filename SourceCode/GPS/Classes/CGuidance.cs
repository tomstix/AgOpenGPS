...
namespace AgOpenGPS
{
    public class CGuidance
    {
        ...
        /// </summary>
        public void CalculateSteerAngle(vec3 pivot, vec3 steer, ref List<vec3> curList, bool _isHeadingSameWay, bool isStanleyMode)
        {
            ...
            if (ptCount > 1)
            {
                double dist, minDistA = double.MaxValue, minDistB = double.MaxValue;
                ...
                if (isStanleyMode)
                {
                    ...
                    if (mf.isReverse) steerHeadingError = -steerHeadingError;
                    ...
                    double XTEc = Math.Atan((distanceFromCurrentLineSteer * mf.vehicle.stanleyDistanceErrorGain) / sped);
                ...
                else
                {
                    if (!mf.yt.isYouTurnTriggered)
                    {
                        ...
                        if (mf.vehicle.purePursuitIntegralGain != 0 && !mf.isReverse)
                        {
                            ...
                            if (mf.isAutoSteerBtnOn && mf.avgSpeed > 2.5 && Math.Abs(derivativeDistError) < 0.1)
                            {
                                ...
                                else if (Math.Abs(distanceFromCurrentLinePivot) > 0.02)
                    ...
                    else
                    {
                        ...
                        {
                            steerAngle = Math.Atan((mf.vehicle.wheelbase * (steerAngle > 0 ? mf.vehicle.maxAngularVelocity : -mf.vehicle.maxAngularVelocity)) / (glm.twoPI * mf.avgSpeed * 0.277777));
                        }
                    }
                }
